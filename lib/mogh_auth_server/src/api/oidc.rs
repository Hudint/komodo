...
pub async fn oidc_callback<I: AuthImpl>(
  ...
  async {
    ...
    let user_id_or_two_factor = match user {
      ...
      Some(user) => {
        if let Some(claim_path) = auth.oidc_groups_attribute_path() {
          let groups = provider
            .get_string_list_claim(&subject, &token, claim_path)
            .await;
          auth
            .sync_oidc_login_groups(user.id().to_string(), groups)
            .await?;
        }
      ...
      None => {
        ...
        if let Some(claim_path) = auth.oidc_groups_attribute_path() {
          let groups = provider
            ...
            .await;
          auth
            .sync_oidc_login_groups(user_id.clone(), groups)
            .await?;
        }
