...
pub trait AuthImpl: Send + Sync + 'static {
  ...
  /// Optional path (dot-separated) to an OIDC claim containing group names.
  /// Example: `groups` or `realm_access.roles`.
  fn oidc_groups_attribute_path(&self) -> Option<&str> {
    None
  }

  /// Sync application authorization from extracted OIDC groups.
  /// Called on successful OIDC login for both existing and newly created users.
