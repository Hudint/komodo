...
impl OidcProvider {
  ...
  pub async fn get_string_list_claim(
    &self,
    subject: &SubjectIdentifier,
    token: &TokenResponse,
    claim_path: &str,
  ) -> Vec<String> {
    let claim_path = claim_path.trim();
    if claim_path.is_empty() {
      return Vec::new();
    }

    let mut values = HashSet::new();
    let segments = claim_path
      .split('.')
    ...
    if let Some(id_token_claims) = token
      .id_token()
      .and_then(raw_id_token_claims)
      .and_then(|value| value.as_object().cloned())
      && let Some(value) = value_at_claim_path(
        &Value::Object(id_token_claims),
        &segments,
      )
    {
      extract_string_list(value, &mut values);
    }
    ...
    let user_info = async {
      self
        ...
        .ok()?
        .request_async::<MapAdditionalClaims, _, CoreGenderClaim>(
          reqwest(self.app_user_agent),
        )
        .await
        .inspect(|user_info| debug!("OIDC USER INFO: {user_info:?}"))
    ...
    if let Some(user_info) = user_info {
      let user_info_value =
        Value::Object(user_info.additional_claims().0.clone());
      if let Some(value) =
        value_at_claim_path(&user_info_value, &segments)
      {
        extract_string_list(value, &mut values);
      }
    }
...
pub struct MapAdditionalClaims(pub serde_json::Map<String, Value>);

impl AdditionalClaims for MapAdditionalClaims {}

fn raw_id_token_claims(
...
fn extract_string_list(value: &Value, list: &mut HashSet<String>) {
  match value {
    Value::String(s) => {
      let s = s.trim();
      if !s.is_empty() {
    ...
    Value::Array(values) => {
      for value in values {
        extract_string_list(value, list);
      }
    }
    _ => {}
  }
